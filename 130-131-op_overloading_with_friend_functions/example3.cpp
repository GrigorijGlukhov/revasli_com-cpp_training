...
public:
  ...
  friend Dollars operator-(const Dollars&, const Dollars&);
  friend Dollars operator+(const Dollars&, int);
  friend Dollars operator+(int, const Dollars&);
};
...
{
  return d1.m_dollars + value; // actually calls operator+(const Dollars, int) here
}
